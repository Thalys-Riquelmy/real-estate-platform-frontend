...
import { AluguelResponse, AluguelRequest, PrestacaoAluguelResponse, PagamentoParcelaRequest } from '../../../../core/models/aluguel.model';
...
export class AlugueisListComponent implements OnInit {
  ...
  abrirModalNovo(): void {
    ...
    // Define data de hoje como padrão para dataInicio
  ...
  confirmarPagamento(): void {
    ...
    this.service.pagarPrestacao(aluguel.id, prestacao.id, dto).subscribe({
      next: () => {
        ...
        this.carregarPrestacoes(aluguel.id);
        this.carregarDados(); // Atualiza a lista principal (caso o status do contrato tenha mudado)
        this.formSubmitting.set(false);
      },
      error: (err) => {
  ...
  // Preenche o valor do aluguel ao selecionar o imóvel
  ...
  getStatusClass(status: string): string {
    const classes: Record<string, string> = {
      ...
      pendente: 'badge-warn', // para parcelas
      pago: 'badge-success', // para parcelas
      atrasado: 'badge-danger' // para parcelas
    };
